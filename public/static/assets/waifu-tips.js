...
// 后端接口
live2d_settings['modelAPI']             = 'https://l2d.lnmpa.top/api/';   // 自建 API 修改这里 //live2d.fghrsh.net/api/
...
live2d_settings['modelTexturesId']      = 14;           // 默认材质 ID，可在 F12 控制台找到
...
live2d_settings['aboutPageUrl']         = 'https://www.google.com.jp';   // 关于页地址, '{URL 网址}'
...
function showMessage(text, timeout, flag) {
    if(flag || sessionStorage.getItem('waifu-text') === '' || sessionStorage.getItem('waifu-text') === null){
        ...
        if (live2d_settings.showF12Message) console.info('[Message]', text.replace(/<[^<>]+>/g,''));
...
function initModel(waifuPath, type) {
    ...
    /* 判断 JQuery */
    if (typeof($.ajax) != 'function') typeof(jQuery.ajax) == 'function' ? window.$ = jQuery : console.warn('[Error] JQuery is not defined.');
    ...
    } catch(err) { console.warn('[Error] JQuery UI is not defined.') }
...
function loadModel(modelId, modelTexturesId=0) {
    ...
    } loadlive2d('live2d', live2d_settings.modelAPI+'get/?id='+modelId+'-'+modelTexturesId, (live2d_settings.showF12Status ? console.info('[Status]','live2d','模型',modelId+'-'+modelTexturesId,'加载完成'):null));
}
